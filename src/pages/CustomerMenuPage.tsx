...
import CustomerAuthGate, { type CustomerInfo } from "@/components/customer/CustomerAuthGate";

export default function CustomerMenuPage() {
  const params = useParams();
  const [searchParams] = useSearchParams();

  const restaurantId = params.restaurantId || searchParams.get("restaurantId") || "";
  const tableNum = params.tableNumber || searchParams.get("table") || "";
  ...
  const { items, addItem, updateQuantity, clearCart, totalAmount, totalItems } = useCart();
  ...
  return (
    <CustomerAuthGate restaurantName={restaurant.name} tableNumber={tableNum} customerInfo={customerInfo} onSubmit={setCustomerInfo}>
      <div className="min-h-screen bg-background pb-24">
        <header className="sticky top-0 z-50 bg-card/80 backdrop-blur-lg border-b border-border px-4 py-4">
          <div className="max-w-lg mx-auto">
            ...
            <p className="text-xs text-muted-foreground">Table {tableNum} · {customerInfo?.name}</p>
          </div>
        </header>
              ...
              <div className="space-y-3">
                {menuItems
                  ...
                  .map((item) => {
                    ...
                    return (
                      <div key={item.id} className="bg-card rounded-xl p-4 border border-border/50 flex justify-between items-start gap-3">
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          {cartItem ? (
                            <>
                              <Button variant="outline" size="icon" className="h-7 w-7 rounded-lg" onClick={() => updateQuantity(item.id, cartItem.quantity - 1)}>
                            ...
                            <Button size="sm" className="h-8 rounded-lg text-xs" onClick={() => addItem(item)}>
        ...
        {totalItems > 0 && !cartOpen && (
          <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 animate-slide-up">
            <Button size="lg" className="rounded-full shadow-lg px-6 gap-2" onClick={() => setCartOpen(true)}>
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-card rounded-t-3xl border-t border-border max-h-[85vh] flex flex-col animate-slide-up">
                ...
                {items.map((item) => (
                  <div key={item.menuItem.id} className="flex items-center justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">${Number(item.menuItem.price).toFixed(2)} each</p>
                    </div>
                    ...
                    <p className="text-sm font-semibold w-16 text-right">${(Number(item.menuItem.price) * item.quantity).toFixed(2)}</p>
                  </div>
                ))}
              </div>
              <div className="p-4 border-t border-border space-y-3">
                ...
                <Button className="w-full gap-2" onClick={handlePlaceOrder} disabled={placingOrder}>
                  ...
                  {placingOrder ? "Placing..." : `Place Order · $${totalAmount.toFixed(2)}`}
                </Button>
              </div>
            </div>
          </div>
